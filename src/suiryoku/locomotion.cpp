...
{
//   mpu = MPU::getInstance();
  walking = std::make_shared<aruku::Walking>();
//   head = Robot::Head::getInstance();
...
bool Locomotion::walk_in_position_until_stop()
{
    ...
    walking->stop();
    while (walking->is_running()) usleep(8000);
...
{
    float delta_direction = alg::deltaAngle(direction, mpu->getAngle());
...
bool Locomotion::move_to_target(float target_x, float target_y)
{
    ...
    float delta_direction = alg::deltaAngle(target_direction, mpu->getAngle());

    // float x_speed = move_max_x;
...
bool Locomotion::rotate_to_target(float target_direction, bool a_move_only)
{
    ...
    if(!(a_move_only)) y_speed = (delta_direction < 0.0) ? move_max_y : -move_max_y;
    float a_speed = (delta_direction < 0.0) ? move_max_a : -move_max_a;
...
{
  float a_speed = alg::mapValue(head->getPanAngle(), -10.0, 10.0, -follow_max_a, follow_max_a);

  float x_speed = alg::mapValue(fabs(a_speed), 0.0, follow_max_a, follow_max_x, 0.);
  x_speed = alg::mapValue(head->getTiltAngle() - min_tilt, 10.0, 0.0, x_speed, 0.0);
  ...
  return (head->getTiltAngle() < min_tilt);
}

// TODO: currently using pan tilt, change to back projection later
bool Locomotion::dribble(float direction)
{
    bool is_dribble = true;

    float pan = head->getPanAngle() + head->getPanCenter();
    float delta_direction = alg::deltaAngle(direction, mpu->getAngle());
...
bool Locomotion::pivot(float direction)
{
    ...
    float pan = head->getPanAngle() + head->getPanCenter();
    float tilt = head->getTiltAngle() + head->getTiltCenter();
...
bool Locomotion::move_to_position_until_pan_tilt(float target_pan, float target_tilt, float direction)
{
    float pan = head->getPanAngle() + head->getPanCenter() + head->getPanError() * 0.5;
    float tilt = head->getTiltAngle() + head->getTiltCenter() + head->getTiltError() * 0.5;
    ...
    printf("pan err %.1f tilt err %.1f\n", head->getPanError(), head->getTiltError());
